...
def update_local_tables(): # on login run this to update local customer and company tables with m8 data 
    ...
    for contact in contacts.data:    
        ...
        try: #django throws exeption if the record does not exist
            ...
            person.type = contact['type']		
            person.is_primary_contact = contact['is_primary_contact']	
            if len(person.first + person.last) >2: # if the name > 2 characters then save it (so we don't save blank or dumb contcact names to db)
                person.save()
    ...
    #now connect contacts wiht companies
    people = Company_Contact.objects.all() #should update this to filter on ony new added contacts
    for p in people:
        #print (p.company_uuid)
        try:
           company_match = Company.objects.get(uuid = p.company_uuid)  
           p.company = company_match
           p.save()
           
        except:
          print ('nomatch') #dont need to do anything   
